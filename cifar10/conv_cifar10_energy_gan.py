...
disc_model.add(nn.Convolutional(filter_size=(4, 4),
                                ...
                                weight=state['d_init'], use_bias=False,
                                name='d_conv3'))
disc_model.add(nn.BatchNorm(state['d_num_filters']*4, name='d_bn3'))
disc_model.add(nn.Expression(T.nnet.relu))
...
disc_model.add(nn.Convolutional(filter_size=(4, 4),
                                ...
                                weight=state['d_init'], use_bias=False,
                                name='d_conv4'))
disc_model.add(nn.BatchNorm(state['d_num_filters']*4, name='d_bn4'))
disc_model.add(nn.Expression(T.nnet.relu))
# out_shape == (b, num_filters, 4, 4)


disc_model.add(nn.Expression(lambda x: T.flatten(x, 2)))
disc_model.add(nn.Linear(state['d_num_filters']*4*4*4, 1, weight=state['d_init'],
                         use_bias=True, name='d_fc5'))
...
infer_model.add(nn.Convolutional(filter_size=(3, 3),
                                 ...
                                 weight=state['d_init'], use_bias=False,
                                 name='q_conv1'))
infer_model.add(nn.BatchNorm(state['d_num_filters'], name='q_bn1'))
...
infer_model.add(nn.Convolutional(filter_size=(4, 4),
                                 ...
                                 weight=state['d_init'], use_bias=False,
                                 name='q_conv2'))
infer_model.add(nn.BatchNorm(state['d_num_filters']*2, name='q_bn2'))
infer_model.add(nn.Expression(T.nnet.relu))
# out_shape == (b, num_filters, 16, 16)
...
infer_model.add(nn.Convolutional(filter_size=(4, 4),
                                 ...
                                 weight=state['d_init'], use_bias=False,
                                 name='q_conv3'))
infer_model.add(nn.BatchNorm(state['d_num_filters']*4, name='q_bn3'))
infer_model.add(nn.Expression(T.nnet.relu))
# out_shape == (b, num_filters, 8, 8)
...
infer_model.add(nn.Convolutional(filter_size=(4, 4),
                                 ...
                                 weight=state['d_init'], use_bias=False,
                                 name='q_conv4'))
infer_model.add(nn.BatchNorm(state['d_num_filters']*4, name='q_bn4'))
infer_model.add(nn.Expression(T.nnet.relu))
# out_shape == (b, num_filters, 4, 4)

infer_model.add(nn.Expression(lambda x: T.flatten(x, 2)))
# parameterizes a Gaussian over latent space: mu and log_sigma
infer_model.add(nn.Linear(state['d_num_filters']*4*4*4, state['noise_size']*2, weight=state['d_init'],
...
gen_model.add(nn.Deconvolutional(filter_size=(4, 4),
                                 ...
                                 weight=state['g_init'],
                                 name='g_deconv4'))
gen_model.add(nn.BatchNorm(state['g_num_filters'], name='g_bn4'))
gen_model.add(nn.Expression(T.nnet.relu))
...
for eidx in range(state['niter'] + state['niter_decay']):
    ...
    if eidx % state['visualize_epoch_freq'] == 0:
        gen_model.evaluate()
        gen_data = np.asarray(gen_sample(noise_vis))
        gen_model.training()
        gen_data = vis_transform(gen_data, state)
        color_grid_vis(gen_data, (10, 50), '%s/samples/samples_%d.png' % (saveto, eidx))
